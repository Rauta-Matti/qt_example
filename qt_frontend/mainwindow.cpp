...
void MainWindow::allPersonsSlot(QNetworkReply *reply)
{
    ...
    if(response_data.compare("-4078")==0 || response_data.compare("")==0){
        ui->labelResault->setText("Virhe tietokantayhteydessä");
    }
    else{
    ...
    ui->labelResault->setText(persons);
    }
...
void MainWindow::on_btnGetOnePerson_clicked()
    ...
    if(id==""){
       ui->labelResault->setText("Anna id !");
    }
    else {
...
void MainWindow::onePersonSlot(QNetworkReply *reply)
    ...
    if(response_data.compare("-4078")==0){
        ...
        ui->labelResault->setText("Henkilöä ei löydy");

        }
     else{
        QJsonDocument json_doc=QJsonDocument::fromJson(response_data);
        QJsonObject json_obj=json_doc.object();
        QString person=QString::number(json_obj["id_person"].toInt())+" "+json_obj["fname"].toString()+" "+ json_obj["lname"].toString()+" "
+QString::number(json_obj["money"].toDouble())+ "\r\n";
        ui->labelResault->setText(person);
        }
...
void MainWindow::raiseSlot(QNetworkReply *reply)
{
    ...
    if(response_data.compare("-4078")==0){
        ...
        ui->labelResault->setText("Nosto ei onnistu");
        }
    else{
         ui->labelResault->setText("Nosto onnistui");
    }
